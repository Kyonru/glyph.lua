...
Standalone examples share a compact runner strip with the example `label` and
runtime status above the runnable demo surface. The `description` remains
available to tours and other tooling without consuming the work viewport. Set
`chrome = false` on an example return table when an example needs to own the
entire viewport.
