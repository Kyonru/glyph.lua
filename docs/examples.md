...
Medieval/MMO inventory demo using `ui.grid`, `ui.spriteSheet`, `onLayout` drop-target bounds, the `potions.png` spritesheet, app-owned drag/drop state, scrollable uniform slots, paged uniform slots, and a variable-size case grid with rejected invalid placements.
