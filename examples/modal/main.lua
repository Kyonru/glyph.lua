...
local function blobTransition(opts)
  ...
  return ui.transitions.custom({
    ...
    draw = function(ctx)
      ...
      local maxRadius = math.sqrt(ctx.bounds.width * ctx.bounds.width + ctx.bounds.height * ctx.bounds.height) / 2 + blob.wobble
...
local function chip(label, color)
  return ui.box({
    ...
    style = {
      ...
      borderColor = { color[1], color[2], color[3], 0.45 },
      borderWidth = 1,
      radius = 20,
    },
  }, {
...
local function FadeModal()
  return ModalBody("TRANSITION / FADE", ACCENT, "Smooth fade in and out", "A quiet default for confirmations, alerts, and debugger panels.", "Got it", "fade")
end

local function SlideModal()
  return ModalBody("TRANSITION / SLIDE", SUCCESS, "Slides in from below", "Good for sheets and pushed interface layers that should feel spatial.", "Dismiss", "slide")
end

local function ScaleModal()
  return ModalBody("TRANSITION / SCALE", { 1.0, 0.72, 0.28, 1 }, "Pops open from the center", "Useful for contextual menus, command palettes, and spotlight dialogs.", "Close", "scale")
end

local function BlobModal()
  return ModalBody("CUSTOM / STENCIL", { 0.92, 0.38, 0.72, 1 }, "Organic blob reveal", "This transition is implemented entirely in the example with love.graphics.stencil.", "Neat, close it", "blob")
end

local function ShaderModal()
  return ModalBody("CUSTOM / SHADER", { 0.55, 0.86, 1.0, 1 }, "Shader-backed transition", "Custom transitions can set shaders, canvases, stencils, transforms, or colors and then draw the layer.", "Done", "shader")
end
...
local function CounterModal()
  ...
  return card(360, {
    ui.column({ gap = 12 }, {
      ...
      ui.text("Each scene or modal layer owns its own hook scope. This counter does not affect the main tree or other modals.", {
        wrap = true,
        width = W - 104,
        style = { color = MUTED },
      }),
    ...
    ui.row({ width = "100%", align = "center", gap = 20 }, {
      ui.button({ label = "-", onClick = function() setCount(count - 1) end }),
...
local function App()
  return ui.stack({
    ...
    ui.column({
      ...
      ui.text("Particles and the moving probe keep running behind every transition.", { style = { color = MUTED } }),
      ui.box({ height = 18 }),
      ui.row({ gap = 10 }, {
        launchBtn("Fade", ACCENT, function()
          ui.modal.open("fade", FadeModal, { transition = ui.transitions.fade({ duration = 0.28 }), width = W, height = H, dismissOnBackdrop = true })
        end),
        launchBtn("Slide", SUCCESS, function()
          ui.modal.open("slide", SlideModal, { transition = ui.transitions.slide({ direction = "bottom", duration = 0.32 }), width = W, height = H, dismissOnBackdrop = true })
        end),
        launchBtn("Scale", { 1.0, 0.72, 0.28, 1 }, function()
          ui.modal.open("scale", ScaleModal, { transition = ui.transitions.scale({ duration = 0.26 }), width = W, height = H, dismissOnBackdrop = true })
        end),
        launchBtn("Blob", { 0.92, 0.38, 0.72, 1 }, function()
          ui.modal.open("blob", BlobModal, { transition = blobTransition(), width = W, height = H, dismissOnBackdrop = true })
        end),
      }),
      ui.row({ gap = 10 }, {
        launchBtn("Shader", { 0.55, 0.86, 1.0, 1 }, function()
          ui.modal.open("shader", ShaderModal, { transition = shaderTransition(), width = W, height = H, dismissOnBackdrop = true })
        end),
        launchBtn("Confirm", DANGER, function()
          ui.modal.open("confirm", ConfirmModal, { transition = ui.transitions.scale({ duration = 0.22 }), width = W, height = 340 })
        end),
        launchBtn("Counter", { 0.55, 0.42, 1.0, 1 }, function()
          ui.modal.open("counter", CounterModal, { transition = "fade", duration = 0.22, width = W, height = 360, dismissOnBackdrop = true })
        end),
      }),
    }),
  })
